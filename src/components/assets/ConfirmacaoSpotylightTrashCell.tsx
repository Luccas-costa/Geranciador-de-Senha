...
interface ConfirmacaoSpotylightTrashCellProps {
  id: number | undefined;
  isLoading: boolean;
  onClose: () => void;
...
export default function ConfirmacaoSpotylightTrashCell({
  onClose,
  handlerDeleteBD,
  id,
  isLoading,
}: ConfirmacaoSpotylightTrashCellProps) {
  return (
    <div className='absolute top-0 left-0 w-full h-full bg-black/50 flex items-center justify-center z-50'>
      <div className='relative flex flex-col space-y-4 transition-all duration-100'>
        ...
        <div className='flex space-x-4'>
          <button
            className='bg-red-900/70 hover:bg-red-900/90 w-[40vw] p-6 rounded-2xl shadow-2xl text-zinc-300'
            onClick={() => id !== undefined && handlerDeleteBD(id)}
          >
            Apagar
          </button>
          <button
            className='bg-green-900/70 hover:bg-green-900/90 w-[40vw] p-6 rounded-2xl shadow-2xl text-zinc-300'
            onClick={onClose}
          >
            Cancelar
          </button>
